  ...
  console.log();
    ...
    return (
      ...
      <div className={`flex flex-col w-full h-full items-center bg-black bg-opacity-75`}>
        ...
        <h2 className="p-10 text-5xl text-center text-green-400">{`Search results for "${location.state}"`}</h2>
        </div>
        <div>
        <ul className="flex flex-col text-white items-center justify-center my-2 mx-0">
        <li className="m-4 py-2 px-10 border-2 border-yellow-400 rounded-lg text-yellow-400 font-bold text-xl">
        Common Name - Scientific Name
        </li>
          {
            results.map(organism => {
              return (
                <li className="m-2 py-2 px-10 border-2 border-green-400 rounded-lg text-green-400 font-bold text-xl">
